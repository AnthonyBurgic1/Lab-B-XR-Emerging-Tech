...
const createScene = async function() {
    ...
    const cart = BABYLON.MeshBuilder.CreateBox("cart", {size: 1});
    ...
    const Wheel1 = BABYLON.SceneLoader.ImportMeshAsync("", "./meshes/", "Wheel1.glb").then((result) => {
        ...
        wheelMesh.scaling = new BABYLON.Vector3(100, 100);
        wheelMesh.rotation = new BABYLON.Vector3(0, 0, BABYLON.Tools.ToRadians(90));

        wheelMesh.parent = cart;
        wheelMesh.position = new BABYLON.Vector3(0.5, -0.25, 0.5);
     }).catch((error) => {
        console.error("Error loading the wheel mesh:", error);
     })
      
 const YelloeStar = BABYLON.SceneLoader.ImportMeshAsync("", "./meshes/", "YelloeStar.glb").then((result) => {
        const YelloeStar = result.meshes[0];
        YelloeStar.position = new BABYLON.Vector3(0, 0, 0);
        YelloeStar.scaling = new BABYLON.Vector3(0.5, 0.5, 0.5);
        YelloeStar.rotation = new BABYLON.Vector3(0, 0, BABYLON.Tools.ToRadians(90));
        YelloeStar.position = new BABYLON.Vector3(zero);
        camera.setTarget(BABYLON.Vector3.Zero());
        camera.target = BABYLON.Vector3.Zero();
        YellowStar.scaling = new BABYLON.Vector3(5, 5, 5);

        }).catch((error) => {
        console.error("Error loading the YelloeStar mesh:", error);
